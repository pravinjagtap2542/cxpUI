...
export class UserProfileService {
  ...
  fetchEnotificationData = 'assets/json/fetchEnotifications.json';
  ...
  //Api call to post push notification--

  postPushNotification(notificationObj): Observable<any> {
    return this.http.post("sendNotification", notificationObj);
  }


  //Api call to fetch enotifications--
  fetchEnotifications() {
    if (this.callLocalJson) {
      return this.http.get<any>(`${this.fetchEnotificationData}`);
    } else {
      return this.http.get(this.apiUrl + "fetchSoldToNumber.json");
    }
  }
